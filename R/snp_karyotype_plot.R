...
karyotype_plt <- function(data, density.col = c("darkgreen","yellow","red"),
                          window.size = 1e6){
  ...
  chrom_key <- setNames(object = as.character(c(1:nrow(sizes))),
                        nm = chrom_order)

  plt_data$chrom <- as.character(plt_data$chrom)
  plt_data <- plt_data %>% merge(sizes, by = "chrom")
  ...
  plt_data <- plt_data %>% mutate(window = (.$chromStart %/% bin) + 1)

  window_cont_df <- as.data.frame(plt_data %>%
                         mutate(window = (.$chromStart %/% bin) + 1) %>%
  ...
  merged_chroms <- unique(merged_data$chrom)
